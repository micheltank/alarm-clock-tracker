...
export default class App extends Component {
        ...
        if (true) { //test
            ...
            BluetoothManager.manager.startDeviceScan(null, null, (error, device) => {
                ...
                if (error) {
                    ...
                    } else if (device.id === 'OUTRO BEACON') {
                        compute(device)
                    }
                }
            })
    ...
    compute = (device) => {
        ...
        if (!this.deviceMap.has(id)) {
            let motion = new MotionAvgFilter(20)
            this.deviceMap.set(id, motion);
        }
        this.deviceMap.get(id).step(device.rssi)
    ...
    report() {
        ...
        this.deviceMap.forEach((value, key) => {
            beacons.push(key, value)
        });
        
        firebase.database().ref(`/beacons/currentpositions`)
            .push({ beacons })
            .then(() => {
                console.log('postado')
            });
